...
class GradCacheTrainer(Trainer):
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        ...
        if not isinstance(self.model.model, supported_classes):
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        ...
        self.model.model.save_pretrained(staging_output_dir, _internal_call=True, from_pt=True)
        ...
        # Save the Task Embedding Layer
        if torch.distributed.get_rank() == 0:
            torch.save(self.model.task_embedding.state_dict(), f'{staging_output_dir}/embedding_weights.pth')
    ...
    def evaluate(self,model,step=500):
        loss_weight = torch.ones(len(ID2TASK)) / len(ID2TASK)
        losses = []
        ...
        task_loss = torch.zeros(len(ID2TASK)).to(self.model.device)
        for valid_step, valid_batch in enumerate(self.get_eval_dataloader()):
            if valid_step > step:
                break
            with torch.no_grad():
                valid_batch = self._prepare_inputs(valid_batch)
                gen_task_id = torch.tensor(valid_batch['gen_task']).to(model.device)
                emb_task_id = torch.tensor(valid_batch['emb_task']).to(model.device)
                gen_loss = model(generative=valid_batch["generative"], gen_task=gen_task_id,loss_weight=loss_weight).loss
                emb_loss = model(query=valid_batch["query"], passage=valid_batch["passage"], emb_task=emb_task_id,loss_weight=loss_weight).loss
                task_loss[gen_task_id] += gen_loss
                task_loss[emb_task_id] += emb_loss
                loss = gen_loss + emb_loss
    ...
    def _inner_training_loop(
        ...
        per_task_weight = torch.ones(len(ID2TASK)).to(self.model.device) / len(ID2TASK)
        if self.weighted_loss_mode == "coba":
            coba_status = CoBaStatus(
                self.coba_warmup_steps,
                self.coba_history_length,
                self.coba_tau,
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                with self.accelerator.accumulate(model):
                    ...
                    if (
                        self.weighted_loss_mode == "coba"
                        and self.accelerator.sync_gradients
                        and self.state.global_step % self.coba_update_interval == 0
                        and self.state.global_step >= self.coba_warmup_steps
                    ):
                        with torch.no_grad():
                            per_task_weight = coba_status.compute_per_task_weight(completed_steps=self.state.global_step)
                            # Smooth
                            alpha = 1
                    ...
                    if self.mode in ["unified", "generative"]:
                        ...
                        if self.no_gen_gas:
                            loss_gen = self.get_loss_no_gas(model=model, generative=inputs["generative"], gen_task=torch.tensor(inputs["gen_task"]), loss_weight=per_task_weight)
                            ...
                            inputs['generative']['gen_task'] = torch.tensor(inputs["gen_task"]).to(inputs["generative"]["input_ids"].device)
                            chunks = gc.split_inputs(inputs["generative"], self.gc_chunk_size)
                            for chunk in chunks:
                                loss_gen_chunk = model(generative=chunk, loss_weight=per_task_weight).loss_gen / len(chunks)
                    ...
                    if self.mode in ["unified", "embedding"]:
                        ...
                        if self.split_emb:
                            ...
                            loss_emb_p, p_reps = self.get_loss_no_gas(
                                ...
                                q_grad=False,
                                get_preps=True,
                                emb_task=torch.tensor(inputs["emb_task"]),
                                #loss_mult=2/3,
                            )
                            ...
                            with self.compute_loss_context_manager():
                                out = model(query=inputs["query"], passage=inputs["passage"], q_grad=False, pos_grad=False, emb_task=torch.tensor(inputs["emb_task"]))
                            ...
                            with self.compute_loss_context_manager():
                                loss = model(q_reps=q_reps, passage=inputs["passage"], p_reps=p_reps, q_grad=False, neg_grad=False, emb_task=torch.tensor(inputs["emb_task"])).loss
                            ...
                            with self.compute_loss_context_manager():
                                loss = model(query=inputs["query"], p_reps=p_reps, pos_grad=False, neg_grad=False, emb_task=torch.tensor(inputs["emb_task"])).loss
                        ...
                        # The below two are incompatible w/ Grad Checkpointing
                            ...
                            loss_emb = self.get_loss_no_gas(model=model, query=inputs["query"], passage=inputs["passage"], emb_task=torch.tensor(inputs["emb_task"]))
                    ...
                    if self.mode == 'unified':
                        ...
                        logger.info(f"Emb_Loss:{loss_emb}")
                ...
                if (
                    ...
                    if self.accelerator.sync_gradients:
                        if (
                            ...
                            coba_status.sample_valid_batch(self.model, self.state.global_step)
                        ...
                        # steps evaluation
                        # if self.state.global_step % self.args.evaluation_steps == 0 and self.valid_dataloader:
                        #     self.model.eval()
                        #     eval_loss, eval_task_loss = self.evaluate(
                        #         self.model,
                        #         loss_weight=per_task_weight,
                        #     )
                        #     self.model.train()
