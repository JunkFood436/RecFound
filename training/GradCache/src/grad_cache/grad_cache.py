...
class GradCache:
    ...
    def cache_step(
        ...
        if no_sync_except_last:
            assert all(map(lambda m: isinstance(m, (FSDP, nn.parallel.DistributedDataParallel)), self.models)), \
        ...
        #     return torch.chunk(torch, num_splits, dim=0)
        # num_splits = model_inputs[0]['input_ids'].shape[0] / self.chunk_sizes
        # query = [{k: split_tensor(v, num_splits)[i] for k, v in model_inputs[0].items()} for i in range(num_splits)] 
        # passage = [{k: split_tensor(v, num_splits)[i] for k, v in model_inputs[1].items()} for i in range(num_splits)] 
        # model_inputs = [(query[i], passage[i]) for i in range(num_splits)]
