...
class CustomDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, item) -> Tuple[BatchEncoding, List[BatchEncoding], BatchEncoding]:
        ...
        if self.mode in ["unified", "embedding"]:
            ...
            emb_task = self.ds_embedding[item]['task']
            id = self.ds_embedding[item]['id'] if 'id' in self.ds_embedding[item].keys() else None
        ...
        if (self.mode in ["unified", "generative"]) and (self.n_samples % self.take_nth == 0):
            ...
            gen_task = self.ds_generative[item]['task']
            id = self.ds_generative[item]['id'] if 'id' in self.ds_embedding[item].keys() else None
        

        self.n_samples -= 1
        return query, passages, generative, emb_task, gen_task, id
...
class CustomCollator(DataCollatorWithPadding):
    ...
    def __call__(self, features):
        ...
        emb_tasks = [f[3] for f in features]
        gen_tasks = [f[4] for f in features]
        ids = [f[5] for f in features]
        ...
        # Used for converting tasks to indexes
        ...
        features["emb_task"] = [task_convert(t) for t in emb_tasks if t is not None]
        features["gen_task"] = [task_convert(t) for t in gen_tasks if t is not None]
        features["id"] = ids
