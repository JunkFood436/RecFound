...
class GritLM(torch.nn.Module):
    def __init__(
        ...
        **kwargs, # Passed to the model, e.g. `attn_implementation`, `torch_dtype` etc.
        ...
        # Embedding Table For Task Types
        self.task_embedding = torch.nn.Embedding(14, 512)
    ...
    def encode(
        ...
        **kwargs,
        ...
        for start_index in tqdm(range(0, len(sentences), batch_size), desc="Batches", disable=len(sentences)<256):
            if isinstance(instruction, list):  # Modified
                sentences_batch = [
                    instr + s + self.embed_eos for instr, s in zip(instruction, sentences[start_index:start_index + batch_size])
            ]
            else:
                sentences_batch = [
